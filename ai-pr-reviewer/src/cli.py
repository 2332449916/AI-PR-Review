"""
CLI entry point for ai-pr-reviewer.

Provides the ``review`` command that accepts a PR URL and orchestrates the
full analysis pipeline: fetch → parse → context-build → analyse → report.
...
Design rationale:
- Using Click rather than argparse for cleaner CLI definitions, nested
  commands, and automatic help generation
- Each CLI option maps directly to a config field in ``AppConfig``
- Rich-based console output provides progress bars and color-coded results
- Tenacity-based retry handles transient API failures transparently
"""
...
def _create_provider(config: AppConfig):
    """Create an LLM provider based on the app config."""
...
def cli(ctx: click.Context, config: str | None, verbose: bool) -> None:
    ...
    Automatically analyses GitHub PRs using LLMs to identify bugs, security
...
def review(
    ...
    """Analyse a GitHub Pull Request and generate a review report.

    PR_URL should be the full URL to the PR, e.g.:
...
def _output_report(report, pr_info, output: str | None, config: AppConfig, fetcher) -> None:
    """Render the analysis report to stdout, a file, and/or a PR comment."""
...
async def _run_analysis(pr_url: str, config: AppConfig) -> tuple:
    ...
    Returns:
        Tuple of (AnalysisReport, PRInfo, GitHubFetcher).
    """
...
def main() -> None:
    """Entry point for the ``ai-pr-reviewer`` console script."""
    cli()
